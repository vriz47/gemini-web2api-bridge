...
def load_cookie() -> tuple:
    ...
    try:
        ...
        xsrf_file = CONFIG.get("xsrf_file")
        if not CONFIG.get("xsrf_token") and xsrf_file and os.path.exists(xsrf_file):
            with open(xsrf_file, "r") as fx:
                token = fx.read().strip()
            if token:
                CONFIG["xsrf_token"] = token
                log("XSRF token dimuat dari " + xsrf_file)
...
def xsrf_from_bytes(raw):
    try:
        m = re.search(rb'\["xsrf","([^"]+)"', raw)
        if m:
...
def save_xsrf_token(token):
    CONFIG["xsrf_token"] = token
    xsrf_file = CONFIG.get("xsrf_file")
    if xsrf_file:
        try:
            with open(xsrf_file, "w") as fx:
                fx.write(token)
        except Exception as e:
            log(f"xsrf save error: {e}")
    log("XSRF token diperbarui (400->retry)")


# ─── Vision helpers (image upload -> file ref via Scotty) ───────────────────

IMAGE_UPLOAD_URL = "https://content-push.googleapis.com/upload/"
_UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"
_TOKEN_CACHE = {"tokens": {}, "ts": 0}


def _page_tokens() -> dict:
    """Scrape push_id/pctx/session_id dari halaman gemini.google.com."""
    ...
    try:
        ...
        log(f"page token gagal: {e}")
        return {}


def _cached_page_tokens() -> dict:
    if time.time() - _TOKEN_CACHE["ts"] > 600:
        _TOKEN_CACHE["tokens"] = _page_tokens()
...
def upload_image(image_bytes: bytes, filename: str = "image.png", mime_type: str = "image/png") -> str:
    """Scotty resumable upload -> file ref (mis. /contrib_service/ttl_1d/...)."""
    ...
    req = urllib.request.Request(IMAGE_UPLOAD_URL, data=b"", headers=start_headers, method="POST")
    resp = urllib.request.urlopen(req, context=ctx, timeout=30)
    upload_url = resp.headers.get("X-Goog-Upload-URL") or resp.headers.get("x-goog-upload-url")
    if not upload_url:
        raise RuntimeError(f"upload: tidak ada X-Goog-Upload-URL ({dict(resp.headers)})")
    ...
    req2 = urllib.request.Request(upload_url, data=image_bytes, headers=up_headers, method="POST")
    resp2 = urllib.request.urlopen(req2, context=ctx, timeout=60)
    file_ref = resp2.read().decode("utf-8", "replace").strip()
    if not file_ref or not file_ref.startswith("/"):
        raise RuntimeError(f"upload: file ref tidak valid: {file_ref[:100]}")
    log(f"upload ok: {filename} -> {file_ref[:50]}...")
    return file_ref


def extract_image_refs(messages: list) -> list:
    """Upload semua part image dari OpenAI messages -> daftar file ref."""
    if not messages:
        return []
    refs = []
    count = 0
    for msg in messages:
        ...
        for c in content:
            if not isinstance(c, dict):
                continue
            if c.get("type") not in ("image_url", "image"):
                continue
            iu = c.get("image_url")
            if isinstance(iu, dict):
                url = iu.get("url")
            else:
                url = c.get("url") or c.get("image")
            if not isinstance(url, str) or not url:
                continue
            data = None
            mime_type = c.get("media_type") or "image/png"
            if url.startswith("data:"):
                m = re.match(r"data:([\w/+-.]+);base64,(.*)", url, re.S)
                if m:
                    mime_type, data = m.group(1), base64.b64decode(m.group(2))
            ...
            if not ext.isalnum():
                ext = "png"
            refs.append(upload_image(data, f"ref{count}.{ext}", mime_type))
    return refs
...
def gemini_stream_generate(prompt: str, model_id: int, think_mode: int, file_refs: list = None) -> str:
    ...
    file_bind = None
    if file_refs:
        file_bind = [[[r], f"ref{i}.png"] for i, r in enumerate(file_refs)]
    inner[0] = [prompt, 0, None, file_bind, None, None, 0]
    inner[1] = ["en"]
        ...
        try:
            ...
            if isinstance(e, urllib.error.HTTPError) and e.code == 400:
                try:
                    ...
                    raw = b""
                tok = xsrf_from_bytes(raw)
                if tok and tok != CONFIG.get("xsrf_token"):
                    save_xsrf_token(tok)
                    continue
            if attempt < CONFIG["retry_attempts"]:
                log(f"Retry {attempt+1}: {e}")
                time.sleep(CONFIG["retry_delay_sec"] if e.__class__.__name__ != "HTTPError" else 0.3)
    raise last_err
...
class GeminiHandler(BaseHTTPRequestHandler):
    ...
    def handle_chat(self, body: bytes):
        ...
        tools = req.get("tools")
        file_refs = extract_image_refs(req.get("messages", []))
